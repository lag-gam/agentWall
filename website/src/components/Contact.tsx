      ...
      <div className="relative z-10 mx-auto max-w-4xl">
        <AnimatedSection className="text-center mb-16">
          ...
          <p className="text-lg text-[#9ca3af]">
            Questions, partnerships, or just want to chat about agent safety.
          </p>
        </AnimatedSection>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {LINKS.map((link, i) => (
            <AnimatedScale key={link.label} delay={i * 0.08}>
              <a
                href={link.href}
                className="glass-card p-6 flex items-center gap-5 group"
              >
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-[#9ca3af] group-hover:text-white group-hover:border-accent/30 transition-all">
                  {link.icon}
                </div>
                <div>
